...
template<class T>
using SYMMETRIC_DIRICHLET_2 = BASE_STORAGE<MATRIX<T, 2>, T, T, T>;

template<class T>
using SYMMETRIC_DIRICHLET_3 = BASE_STORAGE<MATRIX<T, 3>, T, T, T>;

template<class T, int dim>
using SYMMETRIC_DIRICHLET = meta::if_<
meta::equal_to<meta::int_<dim>, meta::int_<2>>,
    SYMMETRIC_DIRICHLET_2<T>, // F, VOL, LAMBDA, MU, _padding_
    SYMMETRIC_DIRICHLET_3<T>  // F, VOL, LAMBDA, MU
>;

template <class T, int dim>
struct SD_HELPER {
    static void Insert(SYMMETRIC_DIRICHLET<T, dim>& sd, int i, T vol, T lambda, T mu) {
        sd.Insert(i, MATRIX<T, dim>(1), vol, lambda, mu);
    }
};

template <class T, int dim>
class SYMMETRIC_DIRICHLET_FUNCTOR {
public:
    using STORAGE = SYMMETRIC_DIRICHLET<T, dim>;
    ...
    static void Append(STORAGE& sd, const VECTOR<int, 2>& handle, T vol, T E, T nu) {
        T lambda = E * nu / (((T)1 + nu) * ((T)1 - (T)2 * nu));
        T mu = E / ((T)2 * ((T)1 + nu));
        for (int i = handle(0); i < handle(1); ++i)
            SD_HELPER<T, dim>::Insert(sd, i, vol, lambda, mu);
    }

    static void Append_FEM(STORAGE& sd, const VECTOR<int, 4>& handle, const SCALAR_STORAGE<T>& vol, T E, T nu) {
        T lambda = E * nu / (((T)1 + nu) * ((T)1 - (T)2 * nu));
        T mu = E / ((T)2 * ((T)1 + nu));
        for (int i = handle(1); i < handle(3); ++i)
            SD_HELPER<T, dim>::Insert(sd, i, std::get<0>(vol.Get_Unchecked_Const(i)), lambda, mu);
    }

    static void Append_All_FEM(STORAGE& sd, const VECTOR<int, 4>& handle, const SCALAR_STORAGE<T>& vol, T E, T nu) {
        T lambda = E * nu / (((T)1 + nu) * ((T)1 - (T)2 * nu));
        T mu = E / ((T)2 * ((T)1 + nu));
    ...
    static void All_Append_FEM(STORAGE& sd, const SCALAR_STORAGE<T>& vol, T E, T nu) {
        T lambda = E * nu / (((T)1 + nu) * ((T)1 - (T)2 * nu));
        T mu = E / ((T)2 * ((T)1 + nu));
    ...
    static void Compute_Kirchoff_Stress(STORAGE& sd, MPM_STRESS<T, dim>& stress) {
        puts("MPM related func, not implemented");
        exit(0);
    }

    static void Compute_Psi(STORAGE& sd, T w, MESH_ELEM_ATTR<T, dim>& elemAttr, T& Psi) {
        TIMER_FLAG("computePsi");
        std::vector<T> psi(elemAttr.size);
        sd.Par_Each([&](const int i, auto data) {
    ...
    static void Compute_Psi(STORAGE& sd, T w, T& Psi) {
        puts("MPM related func, not implemented");
        exit(0);
    }

    static void Compute_First_PiolaKirchoff_Stress(STORAGE& sd, T w, MESH_ELEM_ATTR<T, dim>& elemAttr) {
    ...
    static void Compute_First_PiolaKirchoff_Stress(STORAGE& sd, T w, BASE_STORAGE<MATRIX<T, dim>>& P) {
        puts("MPM related func, not implemented");
        exit(0);
    }

    static void Compute_First_PiolaKirchoff_Stress_Derivative(STORAGE& sd, T w, bool projectSPD, DIFFERENTIAL& dP_div_dF) {
        TIMER_FLAG("computeStressDerivative");
        if(dP_div_dF.size != sd.size) {
            dP_div_dF.Reserve(sd.size);
            sd.Each([&](const int& idx, auto data)
            {
              dP_div_dF.Insert(idx, Eigen::Matrix<T, dim*dim, dim*dim>::Zero());
            });
        }
        sd.Join(dP_div_dF).Par_Each([&](const int i, auto data) {
    ...
    static void Compute_E(const VECTOR<T, dim>& sigma, T u, T lambda, T& E)
    {
        E = 0;
        for (int i = 0; i < dim; ++i) {
            T sigma2 = sigma[i] * sigma[i];
            E += sigma2 + (T)1 / sigma2;
        }
        T kappa = ((T)2 * u + lambda) / (T)8;
        E *= kappa;
    }
    ...
    static void Compute_DE_Div_DSigma(const VECTOR<T, dim>& sigma, T u, T lambda,
                                      VECTOR<T, dim>& dE_div_dsigma)
    {
        dE_div_dsigma = VECTOR<T, dim>();
        for (int i = 0; i < dim; ++i)
            dE_div_dsigma[i] = (T)2 * sigma[i] - (T)2 / (sigma[i] * sigma[i] * sigma[i]);
        T kappa = ((T)2 * u + lambda) / (T)8;
        dE_div_dsigma *= kappa;
    }
    ...
    {
        d2E_div_dsigma2 = MATRIX<T, dim>();
        for (int i = 0; i < dim; ++i)
            d2E_div_dsigma2(i, i) = (T)2 + (T)6 / (sigma[i] * sigma[i] * sigma[i] * sigma[i]);
        T kappa = ((T)2 * u + lambda) / (T)8;
        d2E_div_dsigma2 *= kappa;
    }
                                  ...
                                  VECTOR<T, dim*(dim - 1) / 2>& BLeftCoef)
    {
        ...
        if constexpr (dim == 2) {
            BLeftCoef[0] = 1 + (sigma[0] * sigma[0] + sigma[0] * sigma[1] + sigma[1] * sigma[1]) / (sigma[0] * sigma[0] * sigma[0] * sigma[1] * sigma[1] * sigma[1]);
        }
        else {
            BLeftCoef[0] = 1 + (sigma[0] * sigma[0] + sigma[0] * sigma[1] + sigma[1] * sigma[1]) / (sigma[0] * sigma[0] * sigma[0] * sigma[1] * sigma[1] * sigma[1]);
            BLeftCoef[1] = 1 + (sigma[1] * sigma[1] + sigma[1] * sigma[2] + sigma[2] * sigma[2]) / (sigma[1] * sigma[1] * sigma[1] * sigma[2] * sigma[2] * sigma[2]);
            BLeftCoef[2] = 1 + (sigma[2] * sigma[2] + sigma[2] * sigma[0] + sigma[0] * sigma[0]) / (sigma[2] * sigma[2] * sigma[2] * sigma[0] * sigma[0] * sigma[0]);
        }
        T kappa = ((T)2 * u + lambda) / (T)8;
        BLeftCoef *= kappa;
    }
